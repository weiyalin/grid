...
function GetData() {
    ...
    resultDataTable = $searchResult.dataTable({
        ...
        'bStateSave': true,
        "bServerSide": true,
        "sAjaxSource": '/event_auto_determine_list',
        "language": {
            "sProcessing": "正在加载数据...",
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex) {
            ...
            if ($(nRow).find(".btn-info").length == 0) {
                $(nRow).find("td:last").html("" +
                    "<a class='btn btn-info btn-xs' id='info_edit' v='" + aData.id + "'><i class='fa fa-edit'></i> 编辑 </a>&nbsp;" +
...
function bind() {
    ...
    $("#simpledatatable tbody").on("click", "tr #info_edit", function () {
        var id = $(this).attr('v');
        location.href='/event_auto_determine_view?id='+id;
    });
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.ajax({
                        type: "post",
                        url: '/event_auto_determine_delete',
                        data: {id: id},
