...
function GetData() {
    ...
    resultDataTable = $searchResult.dataTable({
        ...
        'bStateSave': true,
        "bServerSide": true,
        "sAjaxSource": '/event_feedback_determine_list',
        "language": {
            "sProcessing": "正在加载数据...",
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex) {
            ...
            if ($(nRow).find(".btn-info").length == 0) {
                $(nRow).find("td:last").html("" +
                    "<a class='btn btn-info btn-xs' id='info_edit' v='" + aData.id + "'><i class='fa fa-edit'></i> 审核 </a>&nbsp;" +
...
function bind() {
    ...
    $("#simpledatatable tbody").on("click", "tr #info_edit", function () {
       // alert('正在开发中...');
        var id = $(this).attr('v');
        location.href='/event_detail?type=2&id='+id;
    });
