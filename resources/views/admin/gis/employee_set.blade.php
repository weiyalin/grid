            ...
            <form id="population_info" class="form-horizontal" style="padding:20px;">
                ...
                <div class="row">
                    {{--<div class="form-group">
                        ...
                        <div class="col-sm-7">
                            <input value="{{ $employee_info->card_code or null }}" content="{{ $employee_info->population_id or 0 }}" type="text" name="population_id" class="form-control" id="population_id" placeholder="输入证件号"/>
                        </div>
                        <label id="population_id-error" class="error" for="population_id">请正确输入证件号</label>
                    </div>--}}
                    <div class="form-group">
                        <label for="name" class="col-sm-2 control-label">职员姓名：<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <input value="{{ $employee_info->name or null }}" content="{{ $employee_info->population_id or 0 }}"  maxlength="20" type="text" name="name" class="form-control" id="name"/>
                        </div>
                        <label id="name-error" class="error" for="name">职员姓名不可为空</label>
                    </div>
                    <div class="form-group">
                        <label id="default_sex" content="{{ $employee_info->sex or 9 }}" for="sex" class="col-sm-2 control-label">性别：<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                        ...
                        <label id="sex-error" class="error" for="sex">职员性别不可为空</label>
                    </div>
                    <div class="form-group">
                        <label for="post" class="col-sm-2 control-label">工作岗位：<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <input value="{{ $employee_info->post or null }}" maxlength="20" type="text" placeholder="请输入工作岗位" name="post" class="form-control" id="post"/>
                        </div>
                        <label id="post-error" class="error" for="post">工作岗位不可为空</label>
                    </div>
                    <div class="form-group">
                        <label for="unit" class="col-sm-2 control-label">工作单位：<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <input value="{{ $employee_info->unit or null }}" type="text" maxlength="200" placeholder="请输入工作单位" name="unit" class="form-control" id="unit"/>
                        </div>
                        <label id="unit-error" class="error" for="unit">工作单位不可为空</label>
                    </div>
                    <div class="form-group">
                        <label for="age_limit" class="col-sm-2 control-label">工作年限：<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <input value="{{ $employee_info->age_limit or null }}" min="0" max="100000000" type="number" placeholder="请输入工作年限" name="age_limit" class="form-control" id="age_limit"/>
                        </div>
                        <label id="age_limit-error" class="error" for="age_limit">工作年限输入有误</label>
                    </div>
                    <div class="form-group">
                        <label for="mobile" class="col-sm-2 control-label">联系方式：<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <input value="{{ $employee_info->mobile or null }}" type="text" maxlength="20" name="mobile" class="form-control" id="mobile"/>
                        </div>
                        <label id="mobile-error" class="error" for="mobile">联系方式输入有误</label>
                    </div>
                    <div class="form-group">
                        <label id="default_is_party" content="{{ $employee_info->is_party or 0 }}" for="is_party" class="col-sm-2 control-label">是否为党员：<span style="color:#f00">*</span></label>
                        <div class="col-sm-7">
                            <div class="radio-inline">
                                <label>
                ...
                <div class="row" style="margin-top:50px;">
                    <div class="col-md-1 col-md-offset-5">
                        <button class="btn btn-primary btn-lg" employee_id="{{ $employee_info->id or 0 }}" id="employee_set" type="button">
...
@section('footer')
    ...
    <script>
        $(function(){
            ...
            var post_or = true;
            var obj = $("#employee_set").attr('employee_id');
            if(obj!=0){
                $("#name").attr('disabled',true);
            }
            $("#employee_set").click(function(){
                post_or = true;
                ...
                var post = get_data($("#post"),false,true,'post-error');
                var unit = get_data($("#unit"),false,true,'unit-error');
                var age_limit = get_data($("#age_limit"),false,true,'age_limit-error');
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.code==0) {
                            ...
                            setTimeout(function(){
                                window.location.href="/gps_employee";
                            },1000)
                        }else{
