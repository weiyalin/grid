...
function GetData() {
    ...
    resultDataTable = $searchResult.dataTable({
        ...
        'bStateSave': true,
        "bServerSide": true,
        "sAjaxSource": '/event_recycle_bin_list',
        "language": {
            "sProcessing": "正在加载数据...",
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex) {
            ...
            if ($(nRow).find(".btn-info").length == 0) {
                $(nRow).find("td:last").html("" +
                    "<a class='btn btn-info btn-xs' id='restore' v='" + aData.id + "'><i class='fa fa-edit'></i> 还原 </a>&nbsp;" +
                    "<a class='click_delete btn btn-danger btn-xs info_delete' id='info_delete' v='" + aData.id + "'><i class='fa fa-trash-o'></i> 彻底删除 </a>"
                );
            }
            return nRow;
            //添加双击事件
...
function bind() {
    ...
    $("#simpledatatable tbody").on("click", "tr #restore", function () {
        var event_id = $(this).attr('v');
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.ajax({
                        type: 'post',
                        url: "/event_recycle_bin_recorvery",
                        dataType: "json",
                        data: {id:event_id},
